...
export class Formatter extends Lint.Formatters.AbstractFormatter {
  ...
  private testcaseXML(ruleFailure: Lint.RuleFailure): string {
    ...
    return `<testcase time=\"0\" name=\"${ruleName}\">
    <error message=\"${message} (${ruleName})\"><![CDATA[${line}:${character}:${fileName}]]></error>
    </testcase>`;
  }
  ...
  private testsuiteStartXML(failures: Lint.RuleFailure[]): string {
    return `<testsuite time=\"0\" tests=\"${failures.length}\" skipped=\"0\"
     errors=\"${failures.length}\" failures=\"0\" package=\"org.tslint\" name=\"tslint.xml\">`;
  }
